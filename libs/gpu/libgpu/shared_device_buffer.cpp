...
void shared_device_buffer::write(const void *data, size_t size, size_t offset)
{
	if (size == 0)
		return;
...
#ifdef CUDA_SUPPORT
	case Context::TypeCUDA:
		CUDA_SAFE_CALL(cudaMemcpy((char *) cuptr() + offset, data, size, cudaMemcpyHostToDevice));
		break;
#endif
	case Context::TypeOpenCL:
		context.cl()->writeBuffer((cl_mem) data_, CL_TRUE, offset_ + offset, size, data);
		break;
	default:
		gpu::raiseException(__FILE__, __LINE__, "No GPU context!");
	}
...
template<typename T>
void shared_device_buffer_typed<T>::writeN(const T* data, size_t number, size_t offset) {
	this->write(data, number * sizeof(T), offset * sizeof(T));
}
